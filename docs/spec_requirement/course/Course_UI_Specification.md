...
- **Logic**:
  - Uses Jinja2 conditions (`{% if course %}`) to determine if it should post to the create endpoint or the edit endpoint.
  - Pre-fills input values (`value="{{ course.course_name }}"`) if a course object is passed in context.
- **Form Fields**:
  - **Course Name**: `<input type="text" name="course_name" required>`
  - **Technology Stack**: `<input type="text" name="technology_stack" required>`
  - **Duration (Hours)**: `<input type="number" name="duration_hours" required min="1">`
  - **Description**: `<textarea name="description"></textarea>`
  - **Status**: `<select name="status">` with options "Active" and "Inactive".
- **Actions**:
  - Submit Button ("Save Course")
  - Cancel Button (Links back to `/courses/`)

## 4. Flash Messages
- The UI must have a dedicated section (usually in `base.html` or at the top of the content blocks) that iterates over Flask's `get_flashed_messages(with_categories=true)` to display success (green) or error (red) banners.

## 5. Javascript Interactions (`static/js/course.js`)
- **Delete Confirmation**: Prompt users before submitting a delete action (`confirm("Are you sure you want to delete this course?");`).
